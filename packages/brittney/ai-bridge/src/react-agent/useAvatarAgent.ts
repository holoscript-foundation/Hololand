...
export function useAvatarAgent(options: UseAvatarAgentOptions = {}): UseAvatarAgentReturn {
  ...
  useEffect(() => {
    ...
    unsubs.push(
      bridge.on('state:change', ({ current }: { current: AgentState }) => {
    ...
    unsubs.push(
      bridge.on('emotion:change', ({ emotion }: { emotion: AvatarEmotion }) => {
    ...
    unsubs.push(
      bridge.on('emotion:detected', ({ emotion }: { emotion: AvatarEmotion }) => {
        setCurrentEmotion(emotion);
        onEmotionChange?.(emotion);
      })
    );
    ...
    unsubs.push(
      bridge.on('tool:call', (toolCall: AvatarToolCall) => {
    ...
    unsubs.push(
      bridge.on('holoscript:execute', ({ code, action }: { code: string; action: string }) => {
        onHoloScript?.(code, action);
      })
    );

    unsubs.push(
      bridge.on('speech:start', ({ text }: { text: string }) => {
        onSpeech?.(text);
      })
    );

    unsubs.push(
      bridge.on('lipsync:viseme', (data: any) => {
        onViseme?.(data);
      })
    );

    unsubs.push(
      bridge.on('stream:start', () => {
        setIsStreaming(true);
    ...
    unsubs.push(
      bridge.on('text:delta', (delta: string) => {
  ...
  const sendVoice = useCallback(
    async (audio: ArrayBuffer): Promise<void> => {
  ...
  const setEmotion = useCallback(
    (emotion: AvatarEmotion, intensity?: number) => {
