...
// =============================================================================
// NORM PROVENANCE (W.069 Three-Mode Framework)
// =============================================================================

/**
 * Confidence classification for norm provenance, per the W.069 three-mode
 * epistemic framework:
 *
 * - 'genuine':      The norm was verifiably produced through authentic
 *                   agent interaction. The originating agent had access to
 *                   ground truth and the trace faithfully reflects it.
 *
 * - 'confabulated': The norm was generated by an agent filling in gaps with
 *                   plausible but unverified information. The agent *believes*
 *                   the content is accurate but cannot verify it. Common when
 *                   agents interpolate between known data points or reconstruct
 *                   context from partial memory.
 *
 * - 'bullshitted':  The norm was generated without regard for truth or accuracy.
 *                   The originating agent does not care whether the content
 *                   reflects reality -- it was produced to satisfy a social or
 *                   structural requirement (e.g., filling a mandatory field,
 *                   generating plausible-looking data for testing, or producing
 *                   output under time pressure without verification).
 *
 * This distinction matters for collective memory integrity: genuine traces
 * should carry more weight in cluster formation and path preference detection
 * than confabulated or bullshitted traces. Downstream consumers can use this
 * classification to discount or flag low-confidence cultural norms.
 */
export type ConfidenceClassification = 'genuine' | 'confabulated' | 'bullshitted';

/**
 * Provenance record for a cultural norm/trace.
 *
 * Tracks the origin of a trace: which interaction produced it, which agent
 * first established the norm, and how confident we are that the content is
 * authentic. This is critical for epistemic hygiene in multi-agent collective
 * memory systems where traces propagate through reinforcement and diffusion.
 *
 * Without provenance, a confabulated trace reinforced 10 times looks identical
 * to a genuine trace reinforced 10 times. Provenance breaks this symmetry.
 */
export interface NormProvenance {
  /**
   * Unique identifier of the interaction that produced this trace.
   * This links back to the originating event (e.g., a conversation turn,
   * a tool invocation, a world-state observation) for full auditability.
   *
   * Format is application-defined but should be globally unique within
   * the world (e.g., UUID, or `{worldId}:{sessionId}:{sequenceNumber}`).
   */
  originInteractionId: string;

  /**
   * The agent that first established this norm/trace.
   *
   * This may differ from the depositing agent's `agentId` when norms
   * propagate: Agent A establishes a norm, Agent B observes it and
   * deposits a reinforcing trace. The `originatingAgent` remains Agent A
   * so we can trace the epistemic chain back to its source.
   */
  originatingAgent: string;

  /**
   * W.069 three-mode confidence classification.
   * See {@link ConfidenceClassification} for detailed semantics.
   */
...
export interface CulturalTrace {
  ...
  // --- Provenance ---
  /**
   * Norm provenance: records the origin interaction, originating agent, and
   * W.069 confidence classification (genuine/confabulated/bullshitted).
   *
   * Optional for backward compatibility with traces created before provenance
   * tracking was introduced. When absent, consumers should treat the trace as
   * having unknown provenance (equivalent to unclassified confidence).
   */
...
export interface TraceDepositRequest {
  ...
  textContent?: string;
  /**
   * Optional norm provenance for epistemic tracking.
   * Records the origin interaction, originating agent, and W.069 confidence
   * classification. If omitted, the created trace will have no provenance record.
   */
  normProvenance?: NormProvenance;
}
