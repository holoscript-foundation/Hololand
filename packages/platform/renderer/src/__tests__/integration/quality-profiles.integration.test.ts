...
vi.mock('three', async (importOriginal) => {
  ...
  return {
    ...
    WebGLRenderer: vi.fn().mockImplementation(() => ({
      ...
      getSize: vi.fn().mockImplementation((v: any) => { if (v) { v.set(1920, 1080); } return v || { x: 1920, y: 1080 }; }),
      getDrawingBufferSize: vi.fn().mockImplementation((v: any) => { if (v) { v.set(1920, 1080); } return v || { x: 1920, y: 1080 }; }),
      setRenderTarget: vi.fn(),
      getRenderTarget: vi.fn().mockReturnValue(null),
      setClearColor: vi.fn(),
      getClearColor: vi.fn().mockReturnValue({ getHex: () => 0 }),
      ...
      toneMapping: 0,
      outputColorSpace: '',
      info: { render: { calls: 0, triangles: 0, points: 0, lines: 0 }, memory: { geometries: 0, textures: 0 } },
      capabilities: { isWebGL2: true, maxTextures: 16, maxTextureSize: 4096 },
      render: vi.fn(),
      compile: vi.fn(),
