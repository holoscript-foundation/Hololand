...
export const FONT_SIZES = {
  xs: 0.08, sm: 0.1, md: 0.12, lg: 0.16, xl: 0.2, '2xl': 0.28
};
...
export function formatMetricValue(value: number | string, format: HoloMetricData['format'] = 'number'): string {
  if (typeof value === 'string') return value;
  switch (format) {
    case 'percent': return `${value.toFixed(1)}%`;
    case 'currency': return `$${value.toLocaleString()}`;
    case 'duration': return value < 1000 ? `${value}ms` : `${(value/1000).toFixed(1)}s`;
    case 'bytes': return value < 1024 ? `${value}B` : value < 1048576 ? `${(value/1024).toFixed(1)}KB` : `${(value/1048576).toFixed(1)}MB`;
