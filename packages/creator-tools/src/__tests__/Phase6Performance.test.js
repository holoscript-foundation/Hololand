...
import { mockMaterialConfig, mockDevices, mockMetrics, PerformanceMeasure, MemoryMeasure, } from './setup';
describe('Phase 6 Performance Benchmarks', () => {
    ...
    describe('React Component Performance', () => {
        it('should render TraitEditor in <500ms', () => {
            ...
            render(<TraitEditor config={mockMaterialConfig} onCodeChange={() => { }} onMetricsUpdate={() => { }}/>);
            const duration = performanceMeasure.end('render');
            expect(duration).toBeLessThan(500);
        });
        it('should render PreviewDashboard in <500ms', () => {
            const metrics = mockDevices.reduce((acc, device) => ({
                ...acc,
                [device.id]: mockMetrics,
            }), {});
            performanceMeasure.start('render');
            render(<PreviewDashboard devices={mockDevices} metrics={metrics} onMetricsUpdate={() => { }} onRecommendation={() => { }}/>);
            const duration = performanceMeasure.end('render');
            expect(duration).toBeLessThan(500);
        });
        ...
        it('should rerender TraitEditor on props change in <100ms', () => {
            const { rerender } = render(<TraitEditor config={mockMaterialConfig} onCodeChange={() => { }} onMetricsUpdate={() => { }}/>);
            performanceMeasure.start('rerender');
            rerender(<TraitEditor config={{
                    ...mockMaterialConfig,
                    isDirty: true,
                }} onCodeChange={() => { }} onMetricsUpdate={() => { }}/>);
    ...
    describe('Latency Targets', () => {
        ...
        it('should switch views in <200ms', () => {
            const { rerender } = render(<Phase6CompleteDemo config={mockMaterialConfig}/>);
