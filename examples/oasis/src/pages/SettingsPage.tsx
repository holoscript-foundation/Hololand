    ...
    <div className="max-w-4xl mx-auto animate-in">
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 card p-6">
          {activeSection === 'general' && <GeneralSettings theme={theme} setTheme={setTheme} />}
          {activeSection === 'audio' && <AudioSettings />}
          {activeSection === 'graphics' && <GraphicsSettings />}
          {activeSection === 'controls' && <ControlsSettings />}
          {activeSection === 'privacy' && <PrivacySettings />}
          {activeSection === 'account' && <AccountSettings user={user} logout={logout} />}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <SettingRow label="Theme" description="Choose your preferred color scheme">
        <select value={theme} onChange={(e) => setTheme(e.target.value)} className="input w-40">
      ...
      <SettingRow label="Notifications" description="Receive push notifications">
        <Toggle defaultChecked />
      </SettingRow>

      <SettingRow label="Sound Effects" description="Play UI sound effects">
        <Toggle defaultChecked />
      </SettingRow>
    </div>
  );
...
function AudioSettings() {
  return (
    <div className="space-y-6">
      ...
      <SettingRow label="Master Volume" description="Overall volume level">
        <input type="range" min="0" max="100" defaultValue="80" className="w-40" />
      </SettingRow>

      <SettingRow label="Voice Chat" description="Enable voice chat">
        <Toggle defaultChecked />
      </SettingRow>

      <SettingRow label="Microphone" description="Select input device">
        <select className="input w-48">
          <option>Default Microphone</option>
        </select>
      </SettingRow>

      <SettingRow label="Push to Talk" description="Hold key to transmit voice">
        <Toggle />
      </SettingRow>
    </div>
  );
...
function GraphicsSettings() {
  return (
    <div className="space-y-6">
      ...
      <SettingRow label="Quality Preset" description="Overall graphics quality">
        <select className="input w-40">
          <option>Low</option>
      ...
      <SettingRow label="Resolution Scale" description="Render resolution multiplier">
        <select className="input w-40">
          <option>50%</option>
          <option>75%</option>
          <option>100%</option>
      ...
      <SettingRow label="Anti-Aliasing" description="Smooth jagged edges">
        <select className="input w-40">
          <option>Off</option>
          <option>FXAA</option>
          <option>TAA</option>
      ...
      <SettingRow label="V-Sync" description="Synchronize with display refresh rate">
        <Toggle defaultChecked />
      </SettingRow>
    </div>
  );
...
function ControlsSettings() {
  return (
    <div className="space-y-6">
      ...
      <SettingRow label="Mouse Sensitivity" description="Look speed with mouse">
      ...
      <SettingRow label="VR Comfort Mode" description="Reduce motion sickness in VR">
        <Toggle defaultChecked />
      </SettingRow>
...
function PrivacySettings() {
  return (
    <div className="space-y-6">
      ...
      <SettingRow label="Online Status" description="Show when you're online">
        <Toggle defaultChecked />
      </SettingRow>

      <SettingRow label="Activity Status" description="Show what world you're in">
        <Toggle defaultChecked />
      </SettingRow>
      ...
      <SettingRow label="Party Invites" description="Who can invite you to parties">
        <select className="input w-40">
          <option>Everyone</option>
...
function AccountSettings({
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <button className="text-sm text-oasis-error hover:underline">Delete Account</button>
      </div>
    </div>
  );
}
