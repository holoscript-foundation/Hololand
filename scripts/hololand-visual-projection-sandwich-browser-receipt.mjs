...
const SCHEMA = 'hololand.visual-projection-sandwich-browser-receipt.v0.1.0';
const DEFAULT_OUTPUT_DIR = path.join(REPO_ROOT, '.tmp', 'hololand', 'visual-projection-sandwich', 'geolocation-gis-map-room');
...
function usage() {
  return `Usage: node scripts/hololand-visual-projection-sandwich-browser-receipt.mjs [options]

Loads the HoloLand visual projection sandwich render surface in headless Chrome,
clicks the verification control, asserts the HoloScript projection and HoloLand
adapter boundary, and writes a browser receipt.
...
function normalizeUrl(rawUrl) {
  ...
  if (!['http:', 'https:', 'file:'].includes(parsed.protocol)) {
    throw new Error(`Visual projection sandwich URL must be http(s) or file: ${rawUrl}`);
  }
  parsed.hash = '';
...
const STATE_EXPRESSION = `(() => {
  ...
  const gate = window.HOLOLAND_VISUAL_PROJECTION_SANDWICH_GATE || {};
  const interaction = window.HOLOLAND_VISUAL_PROJECTION_SANDWICH_INTERACTION || {};
  const root = document.querySelector('#visual-projection-sandwich-root');
  ...
  const assertions = {
    ...
    packageListVisible: visible('#holoscript-packages') &&
      /@holoscript\\/plugin-geolocation-gis/i.test(text('#holoscript-packages')) &&
      /@holoscript\\/visual/i.test(text('#holoscript-packages')),
    receiptVisible: visible('#visual-sandwich-receipt') && /sourceSemanticsRewritten/i.test(text('#visual-sandwich-receipt')),
    interactionCaptured: document.querySelector('#visual-sandwich-interaction-status')?.dataset?.interacted === 'true' &&
      /Interaction receipt captured/i.test(text('#visual-sandwich-interaction-status')) &&
      interaction.status === 'pass',
    embeddedReceiptPresent: gate.schema === 'hololand.visual-projection-sandwich-gate.receipt.v0.1.0'
  };
  return {
    locationHref: window.location.href,
    title: document.title,
    sourceSha256: root?.dataset?.sourceSha256 || '',
    sourceSemanticsRewritten: root?.dataset?.sourceSemanticsRewritten || '',
    visibleTextSamples: {
      ...
      interaction: text('#visual-sandwich-interaction-status')
    },
    interaction,
    fullTextMentionsSandwich: /visual projection sandwich|hololand visual responsibilities|consumed holoscript packages/i.test(fullText),
    assertions
  };
})()`;
...
function writeBootstrap(jsOutput, receipt) {
  ...
  writeFileSync(
    jsOutput,
    `window.HOLOLAND_VISUAL_PROJECTION_SANDWICH_BROWSER_RECEIPT = ${JSON.stringify(publicReceipt, null, 2)};\n`,
    'utf8',
  );
}
...
async function runReceipt(args) {
  ...
  const profileDir = mkdtempSync(path.join(tmpdir(), 'hololand-visual-sandwich-browser-'));
  const port = 21_000 + Math.floor(Math.random() * 20_000);
  const screenshotPath = path.join(args.outputDir, 'visual-sandwich-gate.png');
  const domPath = path.join(args.outputDir, 'visual-sandwich-gate.dom.html');
  ...
  try {
    ...
    await waitForExpression(
      client,
      `Boolean(window.HOLOLAND_VISUAL_PROJECTION_SANDWICH_GATE && document.querySelector('#visual-sandwich-verify'))`,
      { timeoutMs: args.timeoutMs },
    );
    await evaluate(client, `document.querySelector('#visual-sandwich-verify').click()`);
    await waitForExpression(
      client,
      `document.querySelector('#visual-sandwich-interaction-status')?.dataset?.interacted === 'true'`,
      { timeoutMs: args.timeoutMs },
    );
    ...
    const receipt = {
      ...
      interaction: {
        ...
        selector: '#visual-sandwich-verify',
      },
      state,
      consoleMessages,
      exceptions,
  ...
  try {
    ...
    if (args.json) {
      ...
      process.stdout.write(
        [
          `HoloLand visual projection sandwich browser receipt: ${receipt.status}`,
          `url: ${receipt.url}`,
