...
function fixtureWorkflow() {
  return {
    ...
    modelRoute: { route: 'ollama_cloud', model: 'kimi-cloud' },
    media: { url: 'https://www.youtube.com/watch?v=jfKfPfyJRdk' },
    steps: [
      { id: 'resolve-claude-cli', status: 'resolved', permissionEnvelope: 'read_only', approvalRequired: false, targetResolved: true },
      ...
      { id: 'stage-room-command', status: 'approval_required', permissionEnvelope: 'guarded_execute', approvalRequired: true, targetResolved: true, action: { action: 'type_text', processName: 'WindowsTerminal', text: 'claude "Start /room marathon"' } },
...
function workflowRequest(workflow) {
  ...
  return {
    ...
    model: workflow.summary?.model || workflow.modelRoute?.model || 'kimi-cloud',
    modelRoute: workflow.summary?.modelRoute || workflow.modelRoute?.route || 'ollama_cloud',
